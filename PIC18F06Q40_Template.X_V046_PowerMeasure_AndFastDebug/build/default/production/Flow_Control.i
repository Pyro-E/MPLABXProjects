...
static _Bool s_temp_locked = 0;
static uint32_t s_temp_unlock_ms = 0;
static _Bool s_perm_locked = 0;
static uint32_t s_temp_lock_count = 0;


static _Bool s_valve_should_close = 0;
static _Bool s_valve_cmd_valid = 0;
...
static uint16_t quantize_window(uint16_t window_s)
{
    ...
    uint32_t n = ((uint32_t)window_s * 1000UL) / (uint32_t)10000;
...
static uint32_t sliding_sum(uint16_t window_n)
{
    uint32_t already = FlowLog_GetCaptureCount();
    if (already == 0u) {
        return 0u;
    }
    ...
    for (uint16_t k = 0; k < limit; k++) {
        uint16_t idx = (uint16_t)((w + 1000 - 1u - k) % 1000);
        flowlog_entry_t e;
        FlowLog_GetAt(idx, &e);
        if (e.pulses == 0u) {
            break;
...
{
    _Bool want_close = (s_temp_locked || s_perm_locked);
...
static void evaluate_alerts(void)
{
    ...
    if (!s_temp_locked) {
        uint32_t sum1 = sliding_sum(s_leak1_window_n);
        if (sum1 >= (uint32_t)s_param.leak1_counts) {
            s_temp_locked = 1;
    ...
    if (!s_perm_locked) {
        uint32_t sum2 = sliding_sum(s_leak2_window_n);
        if (sum2 >= (uint32_t)s_param.leak2_counts) {
            s_perm_locked = 1;
        }
    }
...
void FlowControl_Process(void)
{
    ...
    if (s_temp_locked &&
        (timeSpan(s_temp_unlock_ms) >= 30000UL)) {
        s_temp_locked = 0;
        apply_valve();
    }
}
...
{
    uint8_t f = 0u;
    if (s_temp_locked) f |= 0x01u;
    if (s_perm_locked) f |= 0x02u;
    return f;
}
...
void FlowControl_Unlock(uint8_t flags)
{
    if (flags & 0x01u) s_temp_locked = 0;
    if (flags & 0x02u) s_perm_locked = 0;
    apply_valve();
}
