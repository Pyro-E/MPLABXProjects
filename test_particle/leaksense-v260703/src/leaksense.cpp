 ...
 *   FIX A (rollover): hourlyData/dailyGallons now reset on ANY calendar-day
 *          change (dayKey = YYYYMMDD compare), not only when an hour==0 sample
 *          happens to be ingested. Under 24 h power-gating the midnight sample
 *          is routinely skipped, so the old check left stale totals accumulating
 *          across days -- exactly what corrupts a power-on hourlyData test.
 *   FIX B (visibility): during the PIC's INITIAL cold-boot hold we now ALSO
 *          publish "sensorData" (rate-limited, >= INITIAL_HOLD_PUBLISH_MS apart,
 *          only when new samples arrived or a publish was requested). The spec's
 *          hold rules forbid func 0x07 / self-sleep / own power timer -- publish
 *          is allowed, and the cloud is already connected when we enter the hold.
 *          Previously the hold NEVER published, so a cold power-on test showed
 *          "no particle publish" by design.
 *   FIX C (bench escape): the boot REQ_POWER_STATE handshake retried FOREVER,
 *          so a bench P2 without a live PIC never reached loop() (and thus never
 *          published). PIC_HANDSHAKE_MAX_RETRIES (default 0 = infinite, spec
 *          behavior for production) can be set to e.g. 30 for bench bring-up:
 *          after that many 1 s retries we assume NORMAL and proceed.
 * ============================================================================
 *
 * BEGINNER NOTE:
...
// FIX C: boot handshake retry cap. 0 = retry forever (PRODUCTION / spec V049
// behavior: the PIC's own timers cut power if the link is truly dead). Set to a
// small number (e.g. 30) ONLY for bench bring-up with no PIC attached, so the
// firmware eventually assumes NORMAL and continues to the cloud/publish path.
#ifndef PIC_HANDSHAKE_MAX_RETRIES
#define PIC_HANDSHAKE_MAX_RETRIES 6
#endif
...
const unsigned long INITIAL_HOLD_PUBLISH_MS  = 60000;  // FIX B: min gap between publishes during the hold (60 s).
...
// FIX A: lastDayIngested now stores a full calendar dayKey (YYYYMMDD), not a
// day-of-month. -1 still means "none yet". On the FIRST boot after this update
// the old stored day-of-month (1..31) will not match any dayKey, which forces
// exactly one clean rollover reset -- that flushes any stale pre-fix totals.
retained int          lastDayIngested    = -1;     // dayKey (YYYYMMDD) of the last sample processed (-1 = none yet).
...
static inline int dayKeyOf(uint32_t tsUtc);      // FIX A: YYYYMMDD calendar key for a UTC timestamp.
...
void accumulateHourly(int hr, int dayKey, float gallons);   // Add gallons into the right hourly bucket (dayKey=YYYYMMDD).
...
void handleModeButton();                         // Debounced MODE button actions.
...
// FIX A: unique calendar key for a UTC timestamp: year*10000 + month*100 + day
// (e.g. 2026-07-04 -> 20260704). Any change in this key means a new calendar
// day, so the daily reset can never be skipped by missing the hour==0 sample,
// and month/year boundaries are handled for free (unlike a bare day-of-month).
static inline int dayKeyOf(uint32_t tsUtc) {
  return Time.year(tsUtc) * 10000 + Time.month(tsUtc) * 100 + Time.day(tsUtc);
}
...
void ingestPicBatch(const PicSample *s, int n) {
  ...
  for (int k = 0; k < n; k++) {                   // Loop over every sample in the batch.
    ...
    int dayKey = dayKeyOf(tsEnd);                 // FIX A: full YYYYMMDD calendar key (was day-of-month).
    accumulateHourly(hr, dayKey, gallons);        // Add the gallons into the correct hourly bucket.
...
// FIX A: add 'gallons' into the bucket for hour 'hr'; reset hourly/daily totals
// whenever the calendar day changes -- regardless of whether an hour==0 sample
// was ever ingested. Under 24 h power-gating, sessions routinely skip midnight
// entirely (e.g. one session per day at 23:00): the OLD check (hr==0 && day
// changed) then never fired, so dailyGallons/hourlyData accumulated across days
// and any power-on hourlyData test saw stale values. dayKey = YYYYMMDD, so
// month/year boundaries are also safe (a bare day-of-month is not).
void accumulateHourly(int hr, int dayKey, float gallons) {
  if (lastDayIngested == -1) {                    // first ever sample
    lastHourIngested = hr;                        //   Remember this hour as the starting point.
    lastDayIngested  = dayKey;                    //   ...and this calendar day.
  } else if (dayKey != lastDayIngested) {         // ANY calendar-day change = rollover, even if
                                                  //   several whole days were skipped while unpowered.
    Log.info("Day rollover (%d -> %d): daily=%.2f gal reset",   // Log the day's final total before clearing.
             lastDayIngested, dayKey, dailyGallons);
    for (int i = 0; i < 24; i++) hourlyData[i] = 0.0f;   // Clear all 24 hourly buckets for the new day.
    dailyGallons = 0.0f;                          // ONLY reset here (doc 1 bug fixed; now day-robust too).
    imu_data.dailyGallons = 0.0f;                 // Keep the published mirror consistent immediately.
    lastHourIngested = hr;                        // Update the "current hour" marker.
    lastDayIngested  = dayKey;                    // Update the "current day" marker.
  } else if (hr != lastHourIngested) {            // Same day, but we've moved into a new hour...
...
void serviceLocalMeter() {
  ...
  if (gpm > 0 && Time.isValid()) {                // Only proceed if there is real flow and a valid clock.
    ...
    accumulateHourly(Time.hour(now), dayKeyOf(now), gallons);   // FIX A: dayKey here too.
...
// (Kevin's trimmed test payload: renamed short keys, IMU/temp/config/valve
//  fields and the interval-chunk publish commented out for now.)
void imuPublish() {
...
// Debounced MODE button handler.
// Active-low with INPUT_PULLUP: LOW means pressed.
void handleModeButton() {
  bool nowPressed = (digitalRead(MODE_PIN) == LOW);
  unsigned long nowMs = millis();

  if (nowPressed && !lastButtonState && (nowMs - lastPressTime >= DEBOUNCE_TIME)) {
    lastPressTime = nowMs;

    for (int i = 0; i < 24; i++) hourlyData[i] = 0.0f;
    dailyGallons = 0.0f;
    imu_data.dailyGallons = 0.0f;
    lastHourIngested = -1;
    lastDayIngested = -1;

    syncBackupRam();
    triggerPublish = true;
    Log.info("BTN: MODE pressed -> hourlyData reset");
  }
...
// func 0x07, never self-sleep, never run our own timer. We poll the meter every
...
// watched live. The session ends only when the PIC removes power.
//
// FIX B: the hold now ALSO publishes "sensorData" -- rate-limited to one publish
// per INITIAL_HOLD_PUBLISH_MS, and only when new samples arrived this pass or a
// publish was explicitly requested (triggerPublish, e.g. the MODE button or a
// cloud function). This is what makes a COLD power-on hourlyData test visible in
// the Particle console; previously the hold deliberately never published, so the
// test showed nothing. The spec's hold rules (no 0x07, no self-sleep, no own
// power timer) are untouched: publishing is ordinary cloud traffic, and we only
// enter this state from handleConnecting() with the cloud already connected.
void handleInitialHold() {
  ...
  // summary. Hold data is also buffered by the PIC and re-collected in the next normal
  // session, so the periodic publish below is a live mirror, not the system of record.
  uint16_t samplesBefore = gMeter.count;          // FIX B: detect whether this pass ingested anything new.
  ...
  // FIX B: rate-limited live publish so the hold is visible in the Particle console.
  static uint32_t lastHoldPub = 0;                // millis() of the last hold publish (0 = never).
  bool newData   = (gMeter.count != samplesBefore);            // Did this pass ingest fresh samples?
  bool intervalOk = (lastHoldPub == 0) ||                       // First publish of the hold is always allowed...
                    (millis() - lastHoldPub >= INITIAL_HOLD_PUBLISH_MS);   // ...then >= 60 s apart.
  if (Particle.connected() && intervalOk && (newData || triggerPublish)) {
    lastHoldPub = millis();                       // Stamp BEFORE publishing so a slow publish can't double-fire.
    imuPublish();                                 // Same "sensorData" event as a normal session.
    triggerPublish = false;                       // Any pending request is now satisfied.
  }
  // Still deliberately NO sendPhotonOff(), NO sleep, NO timeout: the PIC owns
...
void setup() {
  ...
  // Boot power-state handshake (PIC V049): the FIRST PIC packet must be REQ_POWER_STATE,
  // retried at ~1 Hz until it succeeds, BEFORE any REQ_DATA/GET_PARAM. It tells us whether
  // we are inside the PIC's initial ~10-min cold-boot hold (0 = INITIAL) or in normal
  // operation (1 = NORMAL). Wi-Fi/cloud was kicked off just above and keeps associating in
  // the background during the retry gaps (delay() pumps Particle.process()).
  //
  // FIX C: retry cap. In production (PIC_HANDSHAKE_MAX_RETRIES == 0) this loop retries
  // FOREVER -- correct per spec, because if the link is truly dead the PIC's own timers
  // cut power and we simply re-boot. On a BENCH with no PIC attached, however, "forever"
  // means we never reach loop(), never connect-complete, and never publish -- which looks
  // exactly like "I'm not getting particle publish." Setting the cap (e.g. 30) lets a
  // bench unit assume NORMAL after ~30 s and proceed to the cloud/publish path.
  uint8_t ps = POWER_STATE_NORMAL;                // Safe default if we proceed without an answer.
  uint32_t hsTries = 0;                           // FIX C: handshake attempt counter.
  while (picLink.getPowerState(&ps) != PIC_OK) {  // !=PIC_OK => timeout/CRC/wrong reply: keep retrying.
    Log.warn("PIC power-state handshake failed; retry in 1 s");
    delay(1000);                                  // Wait 1 s (pumps the system) and try again.
#if PIC_HANDSHAKE_MAX_RETRIES > 0
    if (++hsTries >= PIC_HANDSHAKE_MAX_RETRIES) { // FIX C: bench escape hatch only.
      Log.warn("PIC handshake gave up after %lu tries -> assume NORMAL (bench mode)",
               (unsigned long)hsTries);
      break;                                      // 'ps' keeps its POWER_STATE_NORMAL default.
    }
#else
    (void)hsTries;                                // Production: counter unused; retry forever (spec).
#endif
  }
  initialHold = (ps == POWER_STATE_INITIAL);      // Remember which mode the PIC put us in.
  Log.info("PIC power-state = %s", initialHold ? "INITIAL (10-min cold-boot hold)" : "NORMAL");

  triggerPublish = true;                          // This session should publish once (NORMAL path).
  sleepStart = millis(); lastWakeTime = millis(); // Keep timers sane for helpers that read them.
  changeState(STATE_CONNECTING);                  // Let loop() drive: connect -> NORMAL:(report->0x07) / INITIAL:(stream+publish).
  Log.info("Setup complete -> connecting (%s session)",
           initialHold ? "INITIAL-hold" : "normal power-gating");   // Announce setup finished.
#endif
}
...
void loop() {
  ...
  handleModeButton();   // Check MODE button every pass.
  ...
  switch (currentState) {                         // Act based on the current session phase.
    ...
    case STATE_INITIAL_HOLD: handleInitialHold(); break;  // Cold-boot hold: stream + rate-limited publish; PIC cuts power.
