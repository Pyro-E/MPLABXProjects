...
// /* Classified wake reason after STOP sleep. */
// typedef struct {
//     bool  byGpio;
//     pin_t pin;
//     bool  byPic;
...
//     Serial.printf(">>> BOOT  millis=%lu  D10=%d  BTN=%d\r\n",
//                   millis(), (int)digitalRead(PROTO.wakePin), (int)digitalRead(BTN));
// }

// void loop(void) {
//     /* Let WAKE return LOW before re-arming sleep. */
//     {
//         unsigned long t = millis();
//         while (digitalRead(PROTO.wakePin) == HIGH && millis() - t < PROTO.wakeLowWaitMs) delay(5);
//     }

//     Serial.printf(">>> Goto HIBERNATE  millis=%lu  D10=%d\r\n",
//                   millis(), (int)digitalRead(PROTO.wakePin));
//     Serial.flush();

//     SystemSleepConfiguration cfg;
//     cfg.mode(SystemSleepMode::HIBERNATE)
//        .gpio(PROTO.wakePin, RISING);   /* PIC-initiated wake (Case B) */
//     SystemSleepResult r = System.sleep(cfg);
...
//     bool didExchange = false;
//     if (w.byPic) {
//         delay(2);                 /* WAKE already HIGH (rising-edge wake) */
//         uploadRequest(&g_upload); /* Case B */
//         didExchange = true;
//     } else if (w.byButton) {
//         uploadInitiate(&g_upload);/* Case A: 0xF0 -> WAKE HIGH -> 0xAA */
...
//     SystemSleepConfiguration cfg;
//     cfg.mode(SystemSleepMode::ULTRA_LOW_POWER) //SystemSleepMode::STOP
//        .gpio(PROTO.wakePin, RISING)   /* Case B: WAKE rising edge = ACK */
//        .gpio(BTN, FALLING);           /* Case A trigger: external event */
//     SystemSleepResult r = System.sleep(cfg);
