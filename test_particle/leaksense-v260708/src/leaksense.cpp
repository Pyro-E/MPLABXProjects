...
void ingestPicBatch(const PicSample *s, int n, const PicReportInfo &info) {
  ...
  for (int k = 0; k < n; k++) {                   // Loop over every sample in the batch.
    ...
    float freq    = (float)s[k].pulses / PIC_SAMPLE_INTERVAL_SEC_F; // Hz (precise interval)
                                                  //   Frequency = pulses counted / seconds in the interval.
    float gpm     = freqToGpm(freq);              // Convert that frequency to gallons-per-minute.
    float gallons = gpm * (PIC_SAMPLE_INTERVAL_SEC_F / 60.0f) / FLOW_C4;   // Gallons used in this interval (precise interval).
...
#if PIC_MISSED_FILL == PIC_MISSED_FILL_AVERAGE
    if (missedPulses > 0u) {
      ...
      float avgFreq = (float)missedPulses / (float)spanCaps / PIC_SAMPLE_INTERVAL_SEC_F;
      float avgGpm  = freqToGpm(avgFreq);
      float missedGallons = avgGpm
                          * ((float)spanCaps * PIC_SAMPLE_INTERVAL_SEC_F / 60.0f)
                          / FLOW_C4;
...
#ifndef FAST_BENCH_TEST                            // Bench has no network: skip the connect/wait entirely.
...
void setup() {
  ...
  delay(BENCH_SERIAL_SEND_DELAY_MS);               /* step 3 margin: let the PC monitor re-open the COM port */
#endif
  ...
  //
  // We normally get the clock from the cloud, and ingestPicBatch() drops a batch
  // when Time.isValid() is false. With no cloud there is no clock, so seed a LOCAL
  // one here. Rather than the same constant every boot (which piles all data into
  // one hour), we EVOLVE it: read the last virtual time from flash, advance it by
  // one report period plus a small jitter, and save it back. This spreads bench
  // data across hours/days AND makes the per-session sample count drift by a
  // sample or two, exercising the boundary-cut / zero-fill handling.
  uint32_t prevVirt = 0;
  uint32_t virtNow;
  if (loadBlob(BENCHTIME_PATH, &prevVirt, sizeof(prevVirt)) && prevVirt >= FAST_BENCH_TEST_EPOCH) {
    randomSeed(prevVirt ^ millis());              // vary the jitter each session
    int32_t jitter = (int32_t)random(-(int32_t)BENCH_TIME_JITTER_SEC,
                                     (int32_t)BENCH_TIME_JITTER_SEC + 1);
    virtNow = prevVirt + REPORT_PERIOD_SEC + (uint32_t)jitter;   // one period on + jitter
    Log.info("BENCH virtual clock: %lu = prev %lu + period %lu + jitter %ld",
             (unsigned long)virtNow, (unsigned long)prevVirt,
             (unsigned long)REPORT_PERIOD_SEC, (long)jitter);
  } else {
    virtNow = FAST_BENCH_TEST_EPOCH;              // first ever boot: start at the base epoch
    Log.info("BENCH virtual clock: %lu (first boot, base epoch)", (unsigned long)virtNow);
  }
  Time.setTime(virtNow);                          // seed the local clock -> Time.isValid() true
  saveBlob(BENCHTIME_PATH, &virtNow, sizeof(virtNow));   // remember it for the next session
  ...
  Log.info("Setup complete -> FAST_BENCH_TEST (no cloud; evolving local clock; PIC UART + USB log)");
#else
  ...
  // the background during the retry gaps (delay() pumps Particle.process()). If the link is
  // so broken this never succeeds, the PIC's own timers cut power and we simply re-boot.
  uint8_t ps = POWER_STATE_NORMAL;                // Safe default if we somehow proceed.
  while (picLink.getPowerState(&ps) != PIC_OK) {  // !=PIC_OK => timeout/CRC/wrong reply: keep retrying.
    ...
    delay(1000);                                  // Wait 1 s (pumps the system) and try again, indefinitely.
  }
  ...
  sleepStart = millis(); lastWakeTime = millis(); // Keep timers sane for helpers that read them.
  changeState(STATE_CONNECTING);                  // Let loop() drive: connect -> NORMAL:(report->0x07) / INITIAL:(stream).
