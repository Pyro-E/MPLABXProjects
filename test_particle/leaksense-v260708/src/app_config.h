...
// #define FAST_BENCH_TEST 1   // TEST BUILD: no cloud (PIC UART + USB log only). Fast PIC-Photon
                            //  verification -- no cloud-connect delay. COMMENT OUT for the
                            //  production cloud build we ship.

// When FAST_BENCH_TEST is on there is no cloud clock, so we seed a local one at
// boot (see setup()). The absolute value is arbitrary on the bench -- only the
// sample spacing matters -- but it must make Time.isValid() true so batches are
// ingested and logged. 1735689600 = 2025-01-01 00:00:00 UTC.
...
// DEFAULT = PRODUCTION. Define PIC_MODE_TEST for the fast bench build, and then
// build the PIC WITH REPORT_CONFIG_DEBUG. These are the only place the numbers
// live; the precise _F value below is the PIC's real count-multiple window.
//   PRODUCTION : wake 114 counts x 0.5285 s = 60.25 s; capture x4 = 241.004 s;
//                720 samples => ~48 h report.
//   FAST TEST  : wake 6 counts x 0.5285 s = 3.171 s; capture x4 = 12.684 s;
//                10 samples => 126.8 s report.
// #define PIC_MODE_TEST     // define -> fast test; undefined -> PRODUCTION (default)
  ...
  #define PIC_SAMPLES_PER_REPORT     90
#endif
...
constexpr uint32_t METER_INTERVAL_SEC      = PIC_CAPTURE_PERIOD_SEC; // one logger entry per PIC sample (1:1)
constexpr uint32_t PIC_SAMPLE_INTERVAL_SEC = PIC_CAPTURE_PERIOD_SEC; // duration each PIC sample represents
...
constexpr uint16_t SAMPLES_PER_DAY = 24 * 60 * 60 / METER_INTERVAL_SEC; // 86400 / interval
                                   //   test 12 s -> 7200/day; production 240 s -> 360/day.
constexpr uint16_t MAX_SAMPLES     = SAMPLES_PER_DAY;                   // one full day of readings
...
//  SSR valve (D3/D4). They are no longer transmitted to the PIC -- the oldcloudEmit
...
constexpr uint8_t CFG_PUBLISH_HOUR_DFLT = 3;   // Default: 3 AM UTC.
