...
constexpr uint32_t SLEEP_DURATION_S = 15UL * 60UL; //24UL * 60UL * 60UL;  // 24 h publish cadence
...
constexpr uint32_t FIRST_BOOT_AWAKE_MS = 5UL * 60UL * 1000UL;   // stay awake 5 min on first power-up
                                   //   On a cold boot only, stay awake 300,000 ms (= 5 minutes) before allowing sleep.
...
constexpr uint32_t PIC_WAKE_SETTLE_MS     = 20;    // gap after the 0xF0 wake byte
                                                   ...
                                                   //   waking (oscillator restart + return to its main loop)
                                                   //   before we send the actual request. Cheap: only paid once
                                                   //   per PIC transaction, negligible next to the multi-second
                                                   //   REQ_DATA timeout.
