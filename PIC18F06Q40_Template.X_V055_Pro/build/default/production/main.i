...
static uint32_t s_pwr_on_ms = 0;
static uint32_t s_last_valid_ms = 0;
static _Bool s_pkt_seen = 0;
static _Bool s_off_request = 0;
static _Bool s_cold_boot = 0;
...
static _Bool in_initial_hold(void)
{
    return s_cold_boot && (getNowTime() < (uint32_t)(3UL * 60UL * 1000UL));
}
...
void main(void)
{
    ...
    _Bool wdt_reset = (PCON0bits.RWDT == 0);
    PCON0 = 0xFFu;
            ...
            if (in_initial_hold()) {
                ...
                if (UART_TX_IsEmpty() && !FlowReport_IsSending()) {
                    ...
                    s_cold_boot = 0;
                       ...
                       (timeSpan(s_last_valid_ms) >= 20000UL)) {
