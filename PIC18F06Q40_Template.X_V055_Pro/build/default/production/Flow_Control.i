...
static _Bool s_leak1_since_report = 0;
static _Bool s_leak2_since_report = 0;
...
void FlowControl_Init(void)
{
    ...
    s_temp_locked = 0;
    s_perm_locked = 0;
    s_leak1_since_report = 0;
    s_leak2_since_report = 0;
...
void FlowControl_Process(void)
{
    ...
    if (s_temp_locked &&
        (timeSpan(s_temp_unlock_ms) >= 600000UL)) {
        s_temp_locked = 0;
        apply_valve();
    }
}
...
{
    uint8_t f = 0u;
    if (s_leak1_since_report) f |= 0x01u;
    if (s_leak2_since_report) f |= 0x02u;
    return f;
}
