...
# 34 "FlowLog.c"
static uint8_t s_ovf_bits[(1000 + 7u) / 8u];





static uint32_t s_total_at_due = 0;
static uint16_t s_write_at_due = 0;
static uint32_t s_total_at_due2 = 0;
static uint16_t s_write_at_due2 = 0;
static uint8_t s_due_count = 0;

static __attribute__((inline)) void ovf_set(uint16_t slot, _Bool on)
{
...
_Bool FlowLog_DueValid(void) { return s_due_count >= 1u; }
...
void FlowLog_Process(void)
{
    ...
    uint16_t slot = (uint16_t)(s_write % 1000);
    uint16_t delta;
    if (d32 > (uint32_t)16383u) {
        delta = (uint16_t)16383u;
        ovf_set(slot, 1);
    } else {
        delta = (uint16_t)d32;
        ovf_set(slot, 0);
    ...
    s_write = (uint16_t)((s_write + 1u) % 1000);
...
void FlowLog_GetAt(uint16_t index, flowlog_entry_t *out)
{
    uint16_t slot = (uint16_t)(index % 1000);
