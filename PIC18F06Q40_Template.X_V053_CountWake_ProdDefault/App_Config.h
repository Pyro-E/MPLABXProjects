 ...
 * (8-bit Timer0) => max wake ~134.8 s. */
  ...
  #define APP_SAMPLES_PER_REPORT 90u //720u     /* report  = 720 captures = ~48 hours       */
#endif
 ...
 * Photon can update all four at runtime (GET/SET_PARAM). */
  ...
  #define APP_LEAK1_COUNTS_DEF     100u     /* alert1 threshold counts       */
  #define APP_LEAK1_WINDOW_S_DEF   240u     /* alert1 window  (4 min)        */
  #define APP_LEAK2_COUNTS_DEF     500u     /* alert2 threshold counts       */
  #define APP_LEAK2_WINDOW_S_DEF   900u     /* alert2 window  (15 min)        */
  #define TIME_VALVE_TEMP_LOCK_MS  180000UL /* temp lock holds 3 min (180 s) */
#endif
 ...
 *     define TEST_INITIAL_HOLD_10MIN to force the full 10 min even in test. */
  ...
  #else
    #define INITIAL_POWER_HOLD_MS      (30UL * 1000UL)          /* test: 30 seconds */
  #endif
#else
