...
/* Deep-sleep wake period in Timer0 counts. Defaults to the spec value; main()
 * may overwrite it before each sleep with a LOCO-corrected value so the real
 * wake interval tracks 24/48 h despite LFINTOSC drift. Setter keeps it 8-bit. */
static uint8_t s_wake_counts = (uint8_t)APP_FLOW_TMR0H;

void Sys_Time_SetWakeCounts(uint8_t counts)
{
    if (counts == 0u) counts = 1u;      /* never 0 (would be 256-count wrap) */
    s_wake_counts = counts;
}
...
wake_cause_t Sys_Time_EnterDeepSleep(void)
{
    ...
    TMR0H  = s_wake_counts-1;     /* LOCO-corrected wake period (see setter)  */
    TMR0L  = 0x00u;
    PIR3bits.TMR0IF = 0;
    ...
    if (PIR3bits.TMR0IF) {
        MCU_Time_Advance(APP_WAKE_ACTUAL_MS);   /* real count-multiple wake (no quantization) */
