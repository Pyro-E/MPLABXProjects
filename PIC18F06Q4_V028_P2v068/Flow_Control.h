...
/* V023: returns true only if the values were really adopted. A DEBUG build
 * keeps its own bench windows and returns false, so the caller can log
 * IGNORED instead of claiming APPLIED. */
bool     FlowControl_SetParams(const leak_param_t *in);   /* also recalcs + re-evals */
