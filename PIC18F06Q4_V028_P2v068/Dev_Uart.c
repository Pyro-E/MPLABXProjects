 ...
 *  The ISR pushes received bytes here; main() pops and parses.
 * ============================================================ */
static volatile uint8_t  s_rx_buf[UART_RX_BUF_SIZE];
static volatile uint8_t  s_rx_head = 0;   /* write index (ISR)      */
static volatile uint8_t  s_rx_tail = 0;   /* read index  (main)     */
static volatile uint8_t  s_rx_count = 0;  /* bytes currently queued */
...
/* Push one byte into the RX ring (ISR context). Drops on overflow. */
void UART_RX_Push(uint8_t b)
{
    if (s_rx_count < UART_RX_BUF_SIZE) {
        s_rx_buf[s_rx_head] = b;
        s_rx_head = (uint8_t)((s_rx_head + 1u) % UART_RX_BUF_SIZE);
        s_rx_count++;
    }
    /* else: buffer full -> drop (parser will resync / Photon retries) */
}

/* Pop one byte from the RX ring (main context). Returns false if empty. */
bool UART_RX_Pop(uint8_t *b)
{
    bool ok = false;
    INTCON0bits.GIE = 0;            /* brief critical section vs ISR push */
    if (s_rx_count > 0u) {
 ...
 *  Init
 * ============================================================ */
/* V029: see the note in Dev_Uart.h. Only TRIS is touched - the UART peripheral,
 * the PPS routing and the baud generator are all left running, so resuming is a
 * single register write with no re-initialisation and no risk of a half-
 * configured pin. */
void UART_TX_Release(void)
{
    TRISCbits.TRISC0 = 1;      /* input: pin is Hi-Z whatever the peripheral does */
}

void UART_TX_Resume(void)
{
    TRISCbits.TRISC0 = 0;      /* output: U1TX drives it, idle HIGH */
}
...
void UART_ISR(void)
{
    ...
    if (PIE4bits.U1RXIE && PIR4bits.U1RXIF) {
        ...
        /* The callback (main's on_uart_rx) enqueues into the RX ring and
         * asserts WAKE. Kept as a callback so the wake policy lives with the
         * other comms code in main. */
        if (s_rx_cb) {
            s_rx_cb(ch);
        }
    }
