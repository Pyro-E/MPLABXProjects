...
/* Largest inbound payload the PIC must accept. SET_PARAM comes in two forms:
 * len 8 (leak params only) and len 16 (leak params + anchor u32 + interval u32),
 * so this MUST be at least 16 - it was 8, and the receive FSM then rejected
 * every 16-byte SET_PARAM as a bad length and resynced. The Photon logged four
 * unanswered retries before falling back to the 8-byte form, and the handler
 * would also have read data[8..15] past the end of this buffer. Anything longer
 * than a real request is still refused, which is the point of the limit. */
/* Longest inbound payload. With SET_PARAM split, every request is now a FIXED
 * length and the longest is 8 (SET_LEAK, SET_GRID). 16 leaves headroom so a
 * future request does not silently overflow; anything longer than this is
 * still refused, which is the point of the limit. Never reduce this below the
 * longest request or that request will be dropped without any error. */
#define PKT_MAX_RX_DATA     16u
...
#define PKT_RX_BYTE_TIMEOUT_MS  100UL      /* byte-gap: reset parser (anti-noise) */
...
typedef enum {
    ...
    PKT_REQ_SET_LEAK      = 0x03u,  /* write 4 leak params, len 8 -> RSP_ACK/NAK
                                     * (was SET_PARAM, which carried leak params
                                     *  in len 8 AND leak+grid in len 16. Two
                                     *  unrelated quantities in one packet made
                                     *  the LENGTH carry meaning, which collided
                                     *  with the receive length limit and made
                                     *  every 16-byte write vanish silently.
                                     *  Leak and grid are now separate packets,
                                     *  each a fixed length.)                  */
    ...
    PKT_REQ_PHOTON_CFG    = 0x09u,  /* Photon: "give me my config" (no payload)
                                     * -> RSP_PHOTON_CFG. The Photon requests this at
                                     * boot (with retries) so its timing + debug
                                     * settings come from the PIC. If the PIC is not
                                     * configured to provide them it still answers,
                                     * with provided=0, and the Photon uses its own
                                     * compiled defaults. */
    PKT_KEEPALIVE         = 0x0Au,  /* Photon: "alive, still connecting" - NO response.
                                     * Sent periodically while the Photon waits for the
                                     * cloud so the PIC's ACTIVE idle backstop
                                     * (TIMEOUT_NO_MORE_MSG_MS) does not cut power on a
                                     * live-but-connecting Photon. Any CRC-valid packet
                                     * resets that timer; this one carries no meaning
                                     * beyond keeping power (len 0). */

    PKT_DATA_RECEIVED     = 0x0Bu,  /* Photon: "I received & stored the last RSP_DATA
                                     * batch OK" -> PIC COMMITS (advances the FIFO read
                                     * pointer + marks). NO response. This is a DEDICATED
                                     * func, NOT the RSP_ACK (0x8E) used for params.
                                     * Without it a batch is treated as NOT sent
                                     * (consume-on-ACK, see spec 6). len 0. */

    PKT_TIME_SYNC         = 0x0Cu,  /* Photon: absolute cloud time for LOCO self-cal.
                                     * data[0]   = time_valid (1=valid, 0=no cloud time)
                                     * data[1..4]= T_now epoch seconds, big-endian
                                     * (valid only when time_valid=1). len 5.
                                     * PIC recovers wake-abs = T_now - wake->recv delay
                                     * (HFINTOSC) and updates the LOCO factor, then
                                     * replies with PKT_TIME_RECEIVED so the Photon can
                                     * retransmit on loss (dedicated ACK, not RSP_ACK). */

    /* PIC -> Photon */
    PKT_TIME_RECEIVED     = 0x0Du,  /* PIC: "TIME_SYNC received & applied" - dedicated
                                     * reply so the Photon retransmits the time on loss.
                                     * len 0. (Mirror of DATA_RECEIVED, opposite dir.) */

    PKT_REQ_SET_GRID      = 0x0Fu,  /* write report grid, len 8 -> RSP_ACK/NAK
                                     * data: align_anchor u32 (seconds into the
                                     * day, 0 = midnight) + report_interval u32
                                     * (seconds). Nothing to do with leaks: this
                                     * is WHEN the PIC wakes the Photon.        */
    PKT_REQ_GET_LOCO      = 0x0Eu,  /* Photon: "give me LOCO calibration status"
                                     * (no payload) -> RSP_LOCO. Bench readout only. */

    /* PIC -> Photon */
    PKT_RSP_DATA          = 0x81u,  /* data: extended header + samples:
                                     *   u32 impulse_since_report (BE)
                                     *   u32 captures_since_report (BE)
                                     *   u32 impulse_of_span (BE)
                                     *   u16 overflow_count (BE)
                                     *   u32 COUNT (BE)
                                     * then COUNT * 3-byte samples             */
    PKT_RSP_PARAM         = 0x82u,  /* data: 4 x u16 (8 bytes)                 */
    PKT_RSP_VALVE         = 0x84u,  /* data: valve status (8 bytes)            */
    PKT_RSP_LOCO          = 0x8Au,  /* data: LOCO status, 10 bytes big-endian:
                                     *   hf_precision(u16) hf_calibrated(u8)
                                     *   hf_in_range(u8) cloud_calibrated(u8)
                                     *   cloud_precision(u16) cloud_in_range(u8)
                                     *   applied_k(u16). All *_precision/applied_k
                                     *   are Q7 (128 = 1.0). Bench readout only.   */
    PKT_RSP_POWER_STATE   = 0x88u,  /* data[0] = power_state_t (0=initial,1=normal) */
    PKT_RSP_PHOTON_CFG    = 0x89u,  /* data: PhotonCfg block (see App_Config_Photon.h):
                                     *   u8  provided (1=PIC-supplied, 0=use your own)
                                     *   u8  version
                                     *   u32 captureIntervalMs (BE)
                                     *   u16 samplesPerReport   (BE)
                                     *   u8  fastBench
                                     *   u8  debugDataseries
                                     *   u8  missedFillMode
                                     *   u16 serialDelayMs      (BE)   = 13 bytes  */
