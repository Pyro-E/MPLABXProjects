...
/* ---- Test point 1 : RA4 ----
 * DO NOT USE: RA4 is the debug UART transmit pin (U2TX via PPS). Driving it as
 * a GPIO fights the PPS routing and silences the port. */
#define LED_TP1_ON        (LATAbits.LATA4 = 1)
#define LED_TP1_OFF       (LATAbits.LATA4 = 0)
#define LED_TP1_TOGGLE    (LATAbits.LATA4 ^= 1)

/* ---- Test point 2 : RA5 ----
 * DO NOT USE on this board. RA5 is connected to RA4, which carries the debug
 * UART transmit signal, so driving RA5 low also silences that port. LEDs_Init()
 * deliberately leaves the pin unconfigured. These macros are kept only so old
 * code still compiles; using them will break debug output. */
...
//#define PHOTON_EN_DRIVE
/* define -> RC4 drives Photon EN directly (active-HIGH).
 * COMMENTED OUT: this board uses the P-MOS gate (active-LOW,
 * RC4 LOW = ON, HIGH = OFF), matching the client hardware. */
