...
typedef enum {
    VALVE_SELFTEST_IDLE = 0,
    VALVE_SELFTEST_CLOSE,
    VALVE_SELFTEST_CLOSE_WAIT,
    VALVE_SELFTEST_OPEN,
    VALVE_SELFTEST_OPEN_WAIT,
    VALVE_SELFTEST_DONE
} valve_selftest_state_t;
...
void main(void)
{
    ...
    // Valve self-test sequence on cold boot
    valve_selftest_state_t vstate = VALVE_SELFTEST_IDLE;
    if (!wdt_reset) {
        vstate = VALVE_SELFTEST_CLOSE;
    }

    while (vstate != VALVE_SELFTEST_DONE) {
        MValve_OP3_Process();
        switch (vstate) {
            case VALVE_SELFTEST_CLOSE:
                MValve_OP3_CmdClose();
                vstate = VALVE_SELFTEST_CLOSE_WAIT;
                break;
            case VALVE_SELFTEST_CLOSE_WAIT:
                if (!MValve_OP3_IsBusy()) {
                    vstate = VALVE_SELFTEST_OPEN;
                }
                break;
            case VALVE_SELFTEST_OPEN:
                MValve_OP3_CmdOpen();
                vstate = VALVE_SELFTEST_OPEN_WAIT;
                break;
            case VALVE_SELFTEST_OPEN_WAIT:
                if (!MValve_OP3_IsBusy()) {
                    vstate = VALVE_SELFTEST_DONE;
                }
                break;
            default:
                vstate = VALVE_SELFTEST_DONE;
