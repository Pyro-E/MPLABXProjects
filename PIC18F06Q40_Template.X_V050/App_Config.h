...
/* ---- 6. Initial power-hold window. _______ ---------------------------
 * On a COLD power-up (not a WDT/soft reset) the PIC
...
#define INITIAL_POWER_HOLD_MS          (5UL * 60UL * 1000UL)   /* 5 minutes */
 ...
 * of this file; do not redefine it here. The #ifdef below simply consumes it. */
  ...
  #define APP_FLOW_PERIOD_MS  4000          /* fast test: 4 s capture */
  #define APP_FLOW_BATCH      180            /* report every 180 x APP_FLOW_PERIOD_MS = 12 min */
#else
  #define APP_FLOW_SLOTS      1000         /* <=1024 (10-14 sample# limit) */
  #define APP_FLOW_PERIOD_MS  60000UL      /* production: 8 min capture */
  #define APP_FLOW_BATCH      (60*12)      /* 60x12x APP_FLOW_PERIOD_MS = 96 hrs report @ 8 min */
#endif
 ...
 * Photon can update all four at runtime (GET/SET_PARAM). */
  ...
  #define APP_LEAK1_WINDOW_S_DEF   180u     /* alert1 window  (3 min)        */
  #define APP_LEAK2_COUNTS_DEF     900u     /* alert2 threshold counts       */
  #define APP_LEAK2_WINDOW_S_DEF   1800u     /* alert2 window  (30 min)        */
  #define TIME_VALVE_TEMP_LOCK_MS  180000UL /* temp lock holds 3 min        */
#endif
