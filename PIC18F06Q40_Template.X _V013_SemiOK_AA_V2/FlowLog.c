...
/* s_write is free-running; the physical slot is s_write % SLOTS.
 * Free-running indices let (write - sendMark) represent a gap up
 * to 65535, so BATCH may equal SLOTS. */
static uint16_t s_write      = 0;   /* free-running write index */
